...
function sortNewestSongs(songs: Song[]): Song[] {
  return [...songs].sort((a, b) => {
    const firstDate = a.createdAt ?? a.releaseDate
    const secondDate = b.createdAt ?? b.releaseDate
...
export function LandingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([songService.getPublishedSongs(), categoryService.getCategories()])
      ...
      .catch(() => {
        if (isMounted) {
          setErrorMessage('Unable to load music right now. Please try again later.')
        }
      })
      .finally(() => {
        if (isMounted) {
  ...
  const heroSongs = featuredSongs.slice(0, 4)
  const recentlyAddedSongs = sortNewestSongs(songs).slice(0, 4)
  ...
  return (
    <div className="space-y-16">
      ...
      {isLoading ? (
        ...
        <EmptyState title="Could not load music" description={errorMessage} />
      ) : (
        <>
          ...
          <section className="space-y-6">
            ...
            {categories.length > 0 ? (
              ...
              <EmptyState title="No categories yet" description="Categories will appear here when available." />
            )}
          </section>
          ...
          <section className="space-y-6">
            ...
            {recentlyAddedSongs.length > 0 ? (
              ...
              <EmptyState title="No recent songs yet" description="Newest published songs will appear here." />
            )}
          </section>
        </>
      )}
