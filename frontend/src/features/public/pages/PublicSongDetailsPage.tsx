import { Calendar, Clock3, Disc3, Headphones, Lock, Music2 } from 'lucide-react'
...
function getErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : 'Unable to load song details right now.'
}

export function PublicSongDetailsPage() {
      ...
      <section className="grid gap-8 rounded-lg border border-border bg-card/80 p-6 shadow-2xl shadow-black/20 lg:grid-cols-[22rem_1fr]">
        ...
        <div className="flex flex-col justify-center">
          ...
          <dl className="mt-8 grid gap-4 text-sm sm:grid-cols-2">
            <Detail icon={<Disc3 className="size-4" />} label="Album" value={song.album || 'Single'} />
            <Detail icon={<Music2 className="size-4" />} label="Category" value={song.categoryName || 'Music'} />
            <Detail
            ...
            <Detail
              ...
              label="Plays"
              value={song.playCount.toLocaleString()}
            />
          </dl>
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
        <section className="rounded-lg border border-border bg-card/80 p-6">
          ...
          {lyricPreview.length > 0 ? (
            ...
            <p className="mt-6 text-sm leading-6 text-muted-foreground">No lyrics preview available.</p>
          )}
        ...
        <aside className="h-fit rounded-lg border border-primary/30 bg-[linear-gradient(135deg,hsl(346_94%_60%/.2),hsl(235_18%_10%))] p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Sign in or create an account to unlock full playback, favorites, history, and your library.
          </p>
