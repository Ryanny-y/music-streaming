...
import { DataTable, LoadingState, PageHeader, SearchBar, TagBadge, type DataTableColumn } from '@/components/common'
...
export function AdminSongsPage() {
  ...
  const filteredSongs = useMemo(() => {
    ...
    return songs.filter((song) => {
      const matchesSearch =
        !query || [song.title, song.artist, song.album, song.categoryName, ...song.tags].join(' ').toLowerCase().includes(query)
  ...
  const updateStatus = async (song: Song) => {
    ...
    const updatedSong = await adminService.updateSongStatus(song.id, nextStatus)
    setSongs((items) => items.map((item) => (item.id === updatedSong.id ? updatedSong : item)))
  }
  ...
  const columns: DataTableColumn<Song>[] = [
    ...
    { key: 'status', header: 'Status', cell: (song) => <Badge tone={song.status === 'PUBLISHED' ? 'success' : 'muted'}>{song.status}</Badge> },
    { key: 'plays', header: 'Play count', cell: (song) => song.playCount.toLocaleString() },
    {
      ...
      cell: (song) => (
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" size="sm" type="button" onClick={() => navigate(`/admin/songs/${song.id}/edit`)}>
            <Edit className="size-4" />
            Edit
          </Button>
          <Button variant="secondary" size="sm" type="button" onClick={() => updateStatus(song)}>
          ...
          <Button variant="ghost" size="sm" type="button" onClick={() => deleteSong(song)}>
  ...
  return (
    <div className="space-y-8">
      ...
      {isLoading ? <LoadingState label="Loading songs" /> : <DataTable columns={columns} data={filteredSongs} getRowKey={(song) => song.id} />}
    </div>
  )
}
...
function Badge({ children, tone }: { children: ReactNode; tone: 'success' | 'muted' }) {
  const className = tone === 'success' ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-border bg-secondary text-muted-foreground'
